	...
	for(int o=0;o<npoints;o++) 
{
	...
	if(p[0].slope(p[npoints-2])==p[0].slope(p[npoints-1]))
{
	total=total+1;
	//if(p[0].get_x()==p[npoints-1].get_x()&&p[0].get_y()==p[npoints-1].get_y())
	//{
		p[npoints-1].set_x(p[npoints-2].get_x());
		p[npoints-1].set_y(p[npoints-2].get_y());
		//total=total+3;
	npoints=npoints-1;
	}
	...
	int istrapezoid()
	{
	 ...
	 int Inside_Rectangle (point pmax,point pminy)
	 {
		 int po=0;
		 for(int i=0;i<npoints;i++)
		 {
			 if(p[i].get_x()<pminy.get_x()||p[i].get_y()<pminy.get_y()||p[i].get_x()>pmax.get_x()||p[i].get_y()>pmax.get_y())
			 { 
				 po=1;
			 }
	 ...
	 int Inside_Circle (point c,int r)
	 {
		 int tt=0;
		 float d=0;
		 for(int i=0;i<npoints;i++)
	 ...
	 float Area()
	 {
		 float Area=0,U=0,l=0;
		 int u;
		 //Area=((p[0].get_x()*p[1].get_y())-(p[0].get_y()*p[1].get_x()));
		 for(int i=0;i<=npoints;i++)
		 {
			// if(i+1>npoints)
				// break ;
		U=U+(p[i].get_x()*p[i+1].get_y());
		l=l+(p[i].get_y()*p[i+1].get_x());
		u=i;
		 }
		 U=U+(p[u-1].get_x()*p[0].get_y());
		l=l+(p[u-1].get_y()*p[0].get_x());
		 //Area=Area+(-(p[0].get_x()*p[npoints].get_y())+(p[0].get_y()*p[npoints].get_x()));
		 Area=abs(U-l)*0.5;
	  return Area;
	 }
...
//////////////////////////////////////////////////////////// level 3
			...
			if(Q.substr(0,17)=="Inside_Rectangle ")
			{
				point pminy,pmax;
				int po=0,com=0,tt=0;
				string p[1];
				...
				for(int i=0;i<3;i++)
				{
					if(gp[0].get_p(i).get_x()>pmax.get_x()&&gp[0].get_p(i).get_y()>pmax.get_y())
						pmax=gp[0].get_p(i);
					if(gp[0].get_p(i).get_x()<pminy.get_x()&&gp[0].get_p(i).get_y()<pminy.get_y())
						pminy=gp[0].get_p(i);
				}
				for(int i=0;i<Number_Polygons;i++)
				{
					po=g[i].Inside_Rectangle(pmax,pminy);
			if(po==0)
				com++;
			po=0;
				}
			...
			if(Q.substr(0,14)=="Inside_Circle ")
			{
			point p1;
			int r,x,y,coma,lx,ly,coma2,tt=0,d=0,con=0;
			x=Q.find("(");
			coma=Q.find(",");
			y=Q.find(")");
			coma2=Q.find(",",y);
			lx=coma-x-1;
			ly=y-coma-1;
			p1.set_x(atof(Q.substr(x+1,lx).c_str()));
			p1.set_y(atof(Q.substr(coma+1,ly).c_str()));
			r=atoi(Q.substr(coma2+1).c_str());
			for(int i=0;i<Number_Polygons;i++)
			{
				d=g[i].Inside_Circle(p1,r);
				if(d==0)
					con++;
			}
			for(int i=0;i<Number_Polygons;i++)
			...
			if(Q.substr(0,13)=="Polygon_Area ")
			{
				int n=atoi(Q.substr(13).c_str());
			...
			cout<<Ar<<"\n";
				...
				if(Q.substr(0,20)=="Polygons_Area_Range ")
				{
					int coma,com2,con=0,tt=0;
			float min,max;
			coma=Q.find(",");
			com2=20-coma;
			min=atof(Q.substr(20,coma).c_str());
			max=atoi(Q.substr(coma+1).c_str());
			for(int i=0; i<Number_Polygons;i++)
			{
				if(min<=g[i].Area()&& max>=g[i].Area())
					con++;
